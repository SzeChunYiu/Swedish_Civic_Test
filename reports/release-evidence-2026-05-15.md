...
|---|---|
| Date | 2026-05-15 |
| Git commit | `d6e61f9` on `main` (`feat: require concrete submission evidence`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 21:24 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 21:24 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 21:24 CEST:
