...
|---|---|
| Date | 2026-05-15 |
| Git commit | `5808a3e` on `main` (`feat: block dirty release worktrees in preflight`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 22:27 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 22:27 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY; release preflight now also blocks dirty release worktrees before production release actions |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 22:27 CEST:
...
- Release-preflight tests passed: 9/9, including stale public URL evidence, weak READY manual evidence, placeholder/blocker wording, web-draft screenshot evidence, missing store support/privacy URL entry, generic submission evidence, and dirty release worktree fail-closed coverage.
