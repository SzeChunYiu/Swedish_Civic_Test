...
|---|---|
| Date | 2026-05-15 |
| Git commit | `4714ed7` on `main` (`feat: block web draft screenshot evidence`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 21:14 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 21:14 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 21:14 CEST:
...
- Release-preflight tests passed: 6/6, including stale public URL evidence, weak READY manual evidence, placeholder/blocker wording, and web-draft screenshot evidence fail-closed coverage.
