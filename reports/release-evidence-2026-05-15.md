...
|---|---|
| Date | 2026-05-15 |
| Git commit | `adf69e4` on `main` (`feat: guard production submission config`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 21:29 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 21:29 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 21:29 CEST:
...
- Build-config tests passed: 6/6, including production submit guard coverage for placeholder App Store Connect identifiers.
