...
|---|---|
| Date | 2026-05-15 |
| Git commit | `05da3e6` on `main` (`feat: rerun validation before production release actions`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 22:17 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 22:17 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
| Production submit | `npm run submit:production` | Not run; submit guard now blocks unless store submit credentials are concrete and release preflight is fully ready |

## Validation details

Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 22:17 CEST:
