...
|---|---|
| Date | 2026-05-15 |
| Git commit | `bc47a7b` on `main` (`feat: gate production submit on preflight`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 21:49 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 21:49 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 21:49 CEST:
...
- Build-config tests passed: 10/10, including preview build auth guard coverage, production build preflight guard coverage, production submit placeholder-credential guard coverage, and production submit release-preflight guard coverage.
