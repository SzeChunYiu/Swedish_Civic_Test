...
|---|---|
| Date | 2026-05-15 |
| Git commit | `7ba9beb` on `main` (`feat: guard preview build auth`); product commit verified before this evidence refresh |
...
|---|---|---|
| Local validation | `npm run validate` | PASS inside `npm run release:preflight` on 2026-05-15 21:42 CEST; includes typecheck, lint, format check, all test suites, and content validation |
| Release preflight | `npm run release:preflight` | BLOCKED on 2026-05-15 21:42 CEST by external/device/store gates; expected non-zero exit 1; current manual gate evidence lives in `reports/release-gates.json`; public support/privacy URLs are READY |
...
Latest `npm run validate` evidence from `npm run release:preflight` on 2026-05-15 21:42 CEST:
...
- Build-config tests passed: 9/9, including preview build auth guard coverage, production build preflight guard coverage, and production submit guard coverage for placeholder App Store Connect identifiers.
