...
- Private GitHub remote `SzeChunYiu/Swedish_Civic_Test` now exists and tracks `main` plus the batch branch; see `reports/2026-05-15-github-remote.md`.
...
| Apple privacy labels | `publishing/privacy-labels.md`, publishing test, `privacy-review` release-preflight gate | Draft done for current MVP with disabled Google Mobile Ads SDK/test-ID posture disclosed; final store questionnaire review against generated binary is an explicit blocked release gate and local privacy-review JSON is validated for reviewer/time/questionnaire audit trail when referenced |
| Google Play Data Safety | `publishing/google-play-data-safety.md`, publishing test, `privacy-review` release-preflight gate | Draft done for current MVP with Google Mobile Ads SDK test configuration and real-ads-disabled posture disclosed; final Play Console review against generated binary is an explicit blocked release gate and local privacy-review JSON is validated for reviewer/time/questionnaire audit trail when referenced |
...
| Store screenshot manifest and web drafts | `publishing/screenshot-manifest.json`, `reports/2026-05-15-web-draft-screenshots.md`, `publishing/screenshot-shotlist.md`, `scripts/release-preflight.js` final manifest validation | Draft done; final local manifest schema and fail-closed validation done, including pixel width/height and locale; final device screenshots still external |
| Executable release preflight | `scripts/release-preflight.js`, `scripts/release-preflight.test.js`, `scripts/check-public-urls.js`, `scripts/check-public-urls.test.js`, `reports/release-gates.json`, `npm run release:preflight` | Done; runs validation, Expo Doctor, web export, native prebuild, EAS auth, EAS build artifact evidence validation, store submit credential evidence validation, store policy questionnaire evidence validation, live public URL checks, concrete non-placeholder manual-evidence requirements, web-draft screenshot rejection, local device-audio proof artifact validation, local store-record JSON validation, local store account ownership validation, local store listing metadata validation, local final screenshot content-review and pixel/locale metadata validation, local privacy-review audit-trail validation, local submission JSON validation, local monitoring report content validation, store support/privacy URL entry requirements, store policy questionnaire review requirements, final privacy questionnaire review evidence requirements, concrete submission evidence requirements, and manual gate checks; currently reports BLOCKED external gates |
...
| Submit Android app | Requires store account, internal testing, final screenshots/assets; screenshot shotlist exists in `publishing/screenshot-shotlist.md`; local submission JSON schema now validates Android production submission ID/status and monitoring report content when provided; `scripts/submit-production-guard.js` blocks production submit until credentials are concrete and release preflight is fully ready | Not done |
| Submit iOS app | Requires store account, TestFlight, final screenshots/assets; screenshot shotlist exists in `publishing/screenshot-shotlist.md`; local submission JSON schema now validates TestFlight, iOS production submission ID/status, and monitoring report content when provided | Not done |
| Monitor crash reports/content reports/first-week fixes/reviews | Requires launched app and telemetry/support process; release evidence template exists in `reports/release-evidence-template.md`; release preflight now checks local monitoring reports for first-week/crash/content-support/reviews coverage | Not done |
...
3. Review App Store age rating/export compliance and Google Play content rating/ads declarations, then update `store-policy-questionnaires` with concrete evidence; separately re-review Apple privacy labels and Google Play Data safety against the generated binary, including disabled Google Mobile Ads SDK posture, then update `privacy-review`.
4. Upload TestFlight and Google Play internal test builds.
5. Use `reports/release-evidence-template.md`, `publishing/post-eas-auth-runbook.md`, and `publishing/screenshot-shotlist.md` to capture public support/privacy URLs, store-record evidence, device-test evidence, and final screenshots; if store records are local, record them in `reports/store-records/store-records.json` including listing metadata review fields, if device smoke is local, record it in `reports/device-smoke/*.json`, and if screenshots are local, record them in `reports/final-store-screenshots/manifest.json` including pixel width/height and locale, if store policy questionnaire evidence is local, record it in `reports/store-policy-questionnaires/store-policy-questionnaires.json`, if privacy review evidence is local, record it in `reports/privacy-review/privacy-review.json`, and if submission evidence is local, record it in `reports/submission/submission.json` so release preflight can validate them.
