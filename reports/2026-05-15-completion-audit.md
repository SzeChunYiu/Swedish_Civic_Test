...
| Executable release preflight | `scripts/release-preflight.js`, `scripts/release-preflight.test.js`, `scripts/check-public-urls.js`, `scripts/check-public-urls.test.js`, `reports/release-gates.json`, `npm run release:preflight` | Done; runs validation, Expo Doctor, web export, native prebuild, EAS auth, EAS build artifact evidence validation, store submit credential evidence validation, live public URL checks, concrete non-placeholder manual-evidence requirements, web-draft screenshot rejection, local device-audio proof artifact validation, local store-record JSON validation, local store account ownership validation, local store listing metadata validation, local final screenshot content-review and pixel/locale metadata validation, local privacy-review audit-trail validation, local submission JSON validation, local monitoring report content validation, store support/privacy URL entry requirements, final privacy questionnaire review evidence requirements, concrete submission evidence requirements, and manual gate checks; currently reports BLOCKED external gates |
