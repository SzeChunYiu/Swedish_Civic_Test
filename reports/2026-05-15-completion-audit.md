...
5. Use `reports/release-evidence-template.md` and `publishing/screenshot-shotlist.md` to capture public support/privacy URLs, device-test evidence, and final screenshots; if screenshots are local, record them in `reports/final-store-screenshots/manifest.json` so release preflight can validate them.
