...
| EAS/build/native config | `eas.json`, `publishing/build-and-submit-runbook.md`, `scripts/build-config.test.js`, `scripts/native-prebuild-smoke.js`, `scripts/submit-production-guard.js`, isolated native prebuild smoke | Done locally; EAS cloud build still blocked by auth; production build guard blocks production EAS builds until release preflight is ready; production submit guard blocks TBD Apple identifiers and missing Google service-account file |
