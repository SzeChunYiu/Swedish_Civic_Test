...
- Fresh validation result from `npm run validate` on 2026-05-15 21:49 CEST: pass; includes typecheck, lint, format, learning tests, exam tests, audio tests, derived-content test, content-production test, compliance test, monetization test, publishing test, public URL checker test, build-config test, app-assets test, screenshot-manifest test, release-preflight test, theme-discipline test, accessibility-labels test, and content validation.
...
- Executable release preflight exists at `scripts/release-preflight.js`; it now reruns local validation, runs Expo Doctor, runs the web export smoke, runs Android/iOS native prebuild smoke, checks pinned npx EAS CLI/authentication, live-checks hosted support/privacy URLs, requires concrete non-placeholder manual evidence for READY manual gates, rejects web-draft/browser-only screenshots for final screenshot gates, requires store-record evidence to include Support URL and Privacy Policy URL entry, requires concrete submission IDs/URLs/reports, and consumes manual gate evidence from `reports/release-gates.json`. The latest run on 2026-05-15 21:49 CEST at product commit `bc47a7b` still reported `BLOCKED`; local validation, Expo Doctor, web export, native prebuild, pinned npx EAS CLI, and public URLs were ready, while EAS auth, device audio, store records, final screenshots, and submissions still lacked evidence.
...
| EAS/build/native config | `eas.json`, `publishing/build-and-submit-runbook.md`, `scripts/build-config.test.js`, `scripts/native-prebuild-smoke.js`, `scripts/build-preview-guard.js`, `scripts/build-production-guard.js`, `scripts/submit-production-guard.js`, isolated native prebuild smoke | Done locally; EAS cloud build still blocked by auth; preview build guard blocks preview EAS builds until EAS auth is ready; production build guard blocks production EAS builds until release preflight is ready; production submit guard blocks TBD Apple identifiers, missing Google service-account file, and any production submission attempt before release preflight is fully ready |
...
| Submit Android app | Requires store account, internal testing, final screenshots/assets; screenshot shotlist exists in `publishing/screenshot-shotlist.md`; `scripts/submit-production-guard.js` blocks production submit until credentials are concrete and release preflight is fully ready | Not done |
