...
- Executable release preflight exists at `scripts/release-preflight.js`; it now reruns local validation, blocks dirty release worktrees, verifies referenced local artifact paths exist, requires exact hosted support/privacy URL values in READY URL/store evidence, runs the npm moderate-or-higher dependency security audit, runs Expo Doctor, runs the web export smoke, runs Android/iOS native prebuild smoke, checks pinned npx EAS CLI/authentication, live-checks hosted support/privacy URLs, requires concrete non-placeholder manual evidence for READY manual gates, rejects web-draft/browser-only screenshots for final screenshot gates, requires store-record evidence to include Support URL and Privacy Policy URL entry, requires final privacy-questionnaire review evidence to name the generated binary/build and disabled Google Mobile Ads SDK posture, requires concrete submission IDs/URLs/reports, and consumes manual gate evidence from `reports/release-gates.json`. The latest run on 2026-05-15 23:30 CEST at product commit `0ec6044` still reported `BLOCKED`; local validation, Expo Doctor, web export, native prebuild, pinned npx EAS CLI, and public URLs were ready, while EAS auth, device audio, store records, final privacy questionnaire review, final screenshots, and submissions still lacked evidence.
...
5. Use `reports/release-evidence-template.md` and `publishing/screenshot-shotlist.md` to capture public support/privacy URLs, screenshots, and device-test evidence.
