...
- Executable release preflight exists at `scripts/release-preflight.js`; it now reruns local validation, runs Expo Doctor, runs the web export smoke, runs Android/iOS native prebuild smoke, checks pinned npx EAS CLI/authentication, live-checks hosted support/privacy URLs, requires concrete manual evidence for READY manual gates, and consumes manual gate evidence from `reports/release-gates.json`. The latest run on 2026-05-15 21:03 CEST at product commit `56a7e0a` still reported `BLOCKED`; local validation, Expo Doctor, web export, native prebuild, pinned npx EAS CLI, and public URLs were ready, while EAS auth, device audio, store records, final screenshots, and submissions still lacked evidence.
