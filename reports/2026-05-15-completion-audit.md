...
| EAS/build/native config | `eas.json`, `publishing/build-and-submit-runbook.md`, `scripts/build-config.test.js`, `scripts/native-prebuild-smoke.js`, `scripts/build-preview-guard.js`, `scripts/build-production-guard.js`, `scripts/submit-production-guard.js`, isolated native prebuild smoke | Done locally; EAS cloud build still blocked by auth; preview build guard blocks preview EAS builds until EAS auth is ready; production build guard blocks production EAS builds until release preflight is ready; production submit guard blocks TBD Apple identifiers and missing Google service-account file |
...
| Submit Android app | Requires store account, internal testing, final screenshots/assets; screenshot shotlist exists in `publishing/screenshot-shotlist.md`; `scripts/submit-production-guard.js` blocks production submit until credentials are concrete | Not done |
