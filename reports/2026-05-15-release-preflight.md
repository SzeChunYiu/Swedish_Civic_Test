...
- The test suite verifies fail-closed current blockers, the future ready path when EAS/auth and `reports/release-gates.json` evidence are complete, stale public URL evidence blocking when live URL checks fail, weak READY manual evidence blocking when it lacks required concrete details, and READY evidence blocking when it still contains placeholder/blocker wording such as TBD.
