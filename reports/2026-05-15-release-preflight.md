...
- App Store Connect and Google Play Console app record evidence is missing; local store-record JSON evidence is now schema-checked when referenced.
...
- Final store screenshots from an accepted capture method are missing; local final screenshot manifests are now schema-checked when referenced.
- TestFlight, Google Play internal test, production submission, and post-launch
  monitoring evidence are missing; local submission JSON evidence is now schema-checked when referenced.
