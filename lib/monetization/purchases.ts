...
export async function buyRemoveAds({
  ...
  return runWithPurchaseProviderCleanup(provider, async () => {
    ...
    let entitlements: PremiumEntitlements;
    try {
      ...
      return createResult('pending', await getPurchaseEntitlementsSafely(storage), purchase);
    }

    if (!entitlements.adsDisabled) {
      return createResult('pending', entitlements, purchase);
    }
