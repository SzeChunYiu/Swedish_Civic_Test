...
(function () {
  ...
  function render() {
    ...
    reader.innerHTML = `
      ...
      ${titleHtml}
      ${ledeHtml}
      <div class="ebook__body">${bodyHtml}</div>
      <aside class="ebook__notes" aria-label="Your highlights">
        <h4 class="ebook__notes-h">${lang === "sv" ? "Dina markeringar" : "Your highlights"}</h4>
    ...
    // apply highlights + render notes list (ebook-tools.js)
    if (window.smtApplyEbookHighlights) {
      requestAnimationFrame(() => window.smtApplyEbookHighlights());
    }
  }

  // Expose for ebook-tools.js to re-render after highlight changes
