...
(function () {
  ...
  function setSelect(group, value) {
    const el = document.querySelector(`select[data-set="${group}"]`);
  ...
  function syncControls() {
    ...
    setSelect("language",  ls("smt_lang", "en"));
  ...
  document.addEventListener("click", (e) => {
    ...
    // Language menu (topbar globe)
    if (e.target.closest("#lang-open")) {
      const menu = document.getElementById("lang-menu");
      if (menu) menu.hidden = !menu.hidden;
      return;
    }
    const langBtn = e.target.closest("#lang-menu button[data-lang]");
    if (langBtn) {
      const v = langBtn.dataset.lang;
      lsSet("smt_lang", v);
      if (window.applyLang) window.applyLang(v);
      setSelect("language", v);
      const menu = document.getElementById("lang-menu");
      if (menu) menu.hidden = true;
      return;
    }
    // Close lang menu on outside click
    if (!e.target.closest(".lang-pick")) {
      const menu = document.getElementById("lang-menu");
  ...
  document.addEventListener("change", (e) => {
    ...
    if (e.target.matches('select[data-set="language"]')) {
