...
function applyLang(lang) {
  // Merge any stashed extras (loaded before this script)
  if (window.__i18n_extra) {
    Object.assign(i18n, window.__i18n_extra);
...
/* ============================ PRACTICE QUIZ (uses window.SMT_QUESTIONS from questions.js) */

function smtGetQuestions() {
  // Allow practice.js to override (chapter filter or random mix)
  if (typeof window.smtPracticeFilterFor === "function") {
    const filtered = window.smtPracticeFilterFor();
...
function smtQuizRender() {
  ...
  const Qs = smtGetQuestions(); const n = Qs.length;
  ...
  const q = Qs[SMT_QUIZ.i];
  const ans = SMT_QUIZ.answers[SMT_QUIZ.i];
  const answered = ans !== undefined;
  const dots = Array.from({ length: n }, (_, k) => {
    let cls = "";
    if (k < SMT_QUIZ.i) cls = SMT_QUIZ.answers[k] === Qs[k].answer ? "is-right" : "is-wrong";
...
const _smtQuizRenderOriginal = smtQuizRender;
smtQuizRender = function () {
  const hash = (location.hash || "#/").replace(/^#/, "");
  if (hash.startsWith("/practice")) {
    const hasChapter = /[?&]c=/.test(hash);
    if (!hasChapter) {
      // let practice.js render the hub
      return;
    }
    // reset quiz state if we just arrived at a new chapter
    if (!SMT_QUIZ._currentChapterHash || SMT_QUIZ._currentChapterHash !== hash) {
      SMT_QUIZ._currentChapterHash = hash;
      SMT_QUIZ.i = 0; SMT_QUIZ.score = 0; SMT_QUIZ.answers = [];
    }
  }
  return _smtQuizRenderOriginal.apply(this, arguments);
};
...
document.addEventListener("click", (e) => {
  ...
  if (opt && !opt.hasAttribute("disabled")) {
    ...
    if (window.smtRecordAnswer) window.smtRecordAnswer(q.chapterId, correct);
    ...
    if (correct) {
      ...
      for (let i = SMT_QUIZ.i; i >= 0; i--) {
        if (SMT_QUIZ.answers[i] === Qs[i].answer) streak++;
        else break;
      }
      if (streak >= 3 && fx) {
