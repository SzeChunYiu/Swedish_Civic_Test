...
- This branch adds a preview-only Simplified Chinese settings + language-picker UI slice in `lib/localization/zhHansUiPreview.ts` with `scripts/zh-hans-ui-preview.test.js`; it does not enable `zh-Hans`.
...
After the preview-only `zh-Hans` settings + language-picker slice, continue with the next bounded app UI surface before any release enablement:

1. Add a preview-only Simplified Chinese copy slice for the main study entry points: home/profile/practice headers and their accessibility labels.
