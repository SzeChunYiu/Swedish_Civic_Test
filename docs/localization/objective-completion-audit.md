...
- This branch adds preview-only Simplified Chinese dashboard, learning-path, chapter-card, and progress-card labels while keeping `zh-Hans` blocked.
...
After the preview-only `zh-Hans` dashboard/learning slice, continue with the next bounded app UI surface before any release enablement:

1. Add preview-only Simplified Chinese copy for legal/compliance, privacy/local-storage, monetization/paywall, and ad-removal surfaces.
