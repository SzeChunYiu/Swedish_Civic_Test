...
| Actual website/static copy improvement | `site/i18n-extras.js`; `scripts/static-site-i18n-extras-cultural-copy.test.js` | Static extra copy improved for `zh-Hans`, `zh-Hant`, `ar`, `so`, `tr`; guard covers pass/passport slogans, Turkish outcome promises, and Chinese punctuation. | partial |
...
6. Continue replacing static/app mechanical copy for languages beyond the current
   static extra surface (`fa`, `pl`, `ti`, and any future `ckb`/`uk` picker
   addition).
