...
- This branch adds preview-only Simplified Chinese legal/compliance, privacy/local-storage, monetization/paywall, and ad-removal labels while keeping `zh-Hans` blocked.
...
After the preview-only `zh-Hans` compliance/monetization slice, continue with the next bounded app UI surface before any release enablement:

1. Add preview-only Simplified Chinese copy for onboarding/about/support/search/source pages and any remaining high-frequency route headers.
