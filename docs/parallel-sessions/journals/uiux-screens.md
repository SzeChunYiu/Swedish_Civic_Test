...
Screen: Active mock exam question navigator - `app/(tabs)/exam.tsx`, `components/QuestionNavigator.tsx`
Components consumed: exported `QuestionNavigator`, existing `Badge`, `Button`, `ProgressBar`, `QuestionDisclaimer`, `QuestionSourceCitation`, `ExplanationPanel`, and `UHRReferenceCard`.
States covered: active exam question navigation now exposes 20 localized tab states; current question, answered questions, and unanswered questions update from the live exam answers; answer selection also moves the active question state.
A11y: navigator tabs keep localized Swedish/English labels, explicit tab roles, disabled state, and web `aria-selected`; selected question cards receive focus on exported web after navigator selection and keep selected accessibility state.
Verification: required UIUX docs re-read; `./node_modules/.bin/prettier --check 'app/(tabs)/exam.tsx' components/QuestionNavigator.tsx tests/content-exam-route-copy-parity.test.js scripts/ui-effects.test.js` pass; token grep on `app/(tabs)/exam.tsx` and `components/QuestionNavigator.tsx` pass; `NODE_OPTIONS='--v8-pool-size=1' node --test tests/content-exam-route-copy-parity.test.js scripts/ui-effects.test.js` pass 55/55; `NODE_OPTIONS='--v8-pool-size=1' npm run validate:content` pass; `NODE_OPTIONS='--v8-pool-size=1' npm run typecheck -- --pretty false` pass; `NODE_OPTIONS='--v8-pool-size=1' npm run lint` pass; `NODE_OPTIONS='--v8-pool-size=1' npm run test:a11y-labels` pass 1/1; `NODE_OPTIONS='--v8-pool-size=1' npm run test:theme-discipline` pass 1/1; `NODE_OPTIONS='--v8-pool-size=1' npm run test:ownership` pass 1/1; `CI=1 EXPO_NO_TELEMETRY=1 NODE_OPTIONS='--v8-pool-size=1' npm run build:web:export -- --max-workers 2` pass; system Chrome smoke at 390x844 against `/exam` found 20 scoped question tabs, 48x48 tab boxes, `aria-selected=true` moved to question 5, and question 5 focused/scrolled into view.
Blocked? no - scoped to `EXAM-QUESTION-NAVIGATOR-INTEGRATION-1` screen/component/test files plus append-only screen journal.
Next: Queue a focused Playwright regression so the manual exported-web navigator smoke is covered by CI.
