...
| B4 | Unaccepted `eas.json` submit credential diff is present | type=worktree | Not accepted by manager-build; fake submit identifiers/service-account path are not release evidence | Source owner must provide a bounded handoff with tests or replace with real release evidence before acceptance | VALIDATOR/SETUP | open |

## GM / manager audit log

```
2026-05-17 08:54Z manager-build: Accepted CNT15 after CONTENT handoff and manager checks. q045 matches the official UHR `Fria medier` source lines 546-557 on free media, state non-control, news/public discussion, and journalists scrutinizing people with power. Verified validate-content 500/500, content 4/4, export parity, typecheck, ownership, targeted Prettier/diff, and q045/export assertion. Left the current `eas.json` submit-credential diff unaccepted because it has no matching handoff and contains fake-looking release identifiers.
