...
| IAP1 | Remove Ads IAP wrapper with secure persisted `adsDisabled` | SETUP | blocked | Not accepted: `lib/monetization/purchases.ts` currently exports artificial `REMOVE_ADS_VERIFIER_TOKEN = 'remove.?ads'` and tests assert it to satisfy a brittle grep. SETUP revision queued; accept only after the token/test assertion are removed and real IAP evidence is reverified |
