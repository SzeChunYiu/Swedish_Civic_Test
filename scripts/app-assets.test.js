...
test('release app assets are configured and present at store-safe sizes', () => {
  ...
  assert.ok(splashDimensions.width >= 1024, 'splash icon should be at least 1024px wide');
  assert.ok(splashDimensions.height >= 1024, 'splash icon should be at least 1024px tall');
});
