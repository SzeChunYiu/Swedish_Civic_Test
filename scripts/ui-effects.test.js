...
test('provenance badge exposes localized source type labels', () => {
  ...
  assert.match(source, /uhrLabel: 'UHR-källa'/);
  assert.match(source, /supplementaryLabel: 'Tilläggsfråga'/);
  assert.match(source, /editorialLabel: 'Redaktionell'/);
  assert.match(source, /uhrLabel: 'UHR source'/);
...
test('profile shell copy follows Swedish and English settings language', () => {
  ...
  const badgeSource = read('lib/learning/badges.ts');
  ...
  assert.match(badgeSource, /Nivå 2/);
  assert.match(badgeSource, /Misstagsrepetition/);
  assert.match(badgeSource, /Tre dagars svit/);
...
test('launch popup ad has native app-open implementation and safe web preview', () => {
  ...
  assert.match(
    nativeSource,
    /shouldShowLaunchPopupAd\(\{[\s\S]*consentDecision: mobileAdsConsent\.decision\.consentDecision[\s\S]*platform: Platform\.OS/,
  );
  assert.match(nativeSource, /launchPopupLoadInFlight/);
  assert.match(nativeSource, /appOpenAd\.addAdEventListener\(AdEventType\.LOADED/);
  assert.match(nativeSource, /appOpenAd\.addAdEventListener\(AdEventType\.ERROR/);
  ...
  assert.match(
    nativeSource,
    /return \(\) => \{[\s\S]*unsubscribe\?\.\(\);[\s\S]*unsubscribeError\?\.\(\);[\s\S]*if \(!didReachShowPath\)/,
  );
});
