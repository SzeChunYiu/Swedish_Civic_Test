...
test('question card groups prompt and translation into an accessible summary', () => {
  ...
  assert.match(source, /const provenanceLabel = getQuestionProvenanceLabel\(question\);/);
  ...
  assert.match(source, /Source type: \$\{provenanceLabel\}/);
  assert.match(source, /`Source citation: \$\{sourceCitation\}`/);
  assert.match(helperSource, /Källa\/Source: Sverige i fokus/);
  assert.match(helperSource, /External source - beyond the UHR material/);
  ...
  assert.match(source, /<Text style=\{styles\.provenanceLabel\}>\{provenanceLabel\}<\/Text>/);
