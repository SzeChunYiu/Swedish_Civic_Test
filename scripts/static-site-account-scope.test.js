...
test('static site optional account surface keeps ebook highlights account-free', () => {
  ...
  assert.match(signin, /function\s+hasSupabaseCallbackParams\(\)/);
  assert.match(
    signin,
    /if\s*\(!isConfigured\(\)\)\s+return;[\s\S]*clearConfiguredLocalDemoSession\(\);[\s\S]*if \(!hasSupabaseCallbackParams\(\) && !hasStoredRealAccountSession\(\)\) return;[\s\S]*getClient\(\)\.then/,
  );
