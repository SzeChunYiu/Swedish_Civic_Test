...
test('full content production validates source-backed UHR and external questions', () => {
  ...
  assert.equal(summary.questions, 520);
  assert.equal(summary.publishedQuestions, 520);
  ...
  assert.equal(summary.sourceQuestions, 104);
  assert.equal(summary.generatedPublishedQuestions, 416);
  ...
  assert.equal(summary.uhrProvenanceQuestions, 500);
  assert.equal(summary.externalProvenanceQuestions, 20);
  assert.equal(summary.sourceReferencesValidated, summary.publishedQuestions);
  assert.equal(summary.externalSourceReferencesValidated, summary.externalProvenanceQuestions);
  assert.equal(summary.externalProvenanceUiLabelParityValidated, true);
