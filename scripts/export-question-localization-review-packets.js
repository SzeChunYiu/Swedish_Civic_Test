...
function assertQuestionReadinessFailClosed() {
  ...
  for (const locale of targetLocales) {
    ...
    if (
      ...
      readinessEntry.questionContent !== 'pilot_q001_q172_machine_assisted' ||
      readinessEntry.nativeReview !== 'missing' ||
      readinessEntry.releaseGate !== 'blocked'
    ) {
      throw new Error(
        `${locale} question readiness must remain fail-closed (appAvailable=false, questionContent=pilot_q001_q172_machine_assisted, nativeReview=missing, releaseGate=blocked)`,
      );
    }
  }
}
...
function buildPackets() {
  ...
  if (ids.join('\n') !== expectedIds.join('\n')) {
    throw new Error(`Expected q001-q172 UHR questions, found ${ids.length}: ${ids.join(', ')}`);
  }
...
function main() {
  ...
  console.log(
    `Question localization review packets ${checkMode ? 'OK' : 'exported'} (${targetLocales.length} locales, 172 questions)`,
  );
}
