...
function makeTempOutputDir(t) {
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sct-ux-sim-'));
  t.after(() => {
...
function runSimulatorWithTempOutput({
  ...
  try {
    ...
    throw new Error(`UX simulation temp run failed for ${outputDir}.${stdout}${stderr}`);
  }
}
...
test('UX simulation temp-output fixtures register cleanup after assertions', () => {
  const testSource = readRepoFile('scripts/ux-simulation.test.js');

  assert.match(testSource, /function makeTempOutputDir\(t\)/);
  assert.match(testSource, /t\.after\(\(\) => \{/);
  assert.match(testSource, /fs\.rmSync\(outputDir, \{ recursive: true, force: true \}\)/);
  assert.match(testSource, /UX simulation temp run failed for \$\{outputDir\}/);
});

test('UX simulation temp output preserves existing generatedAt and canonical markdown', (t) => {
  const outputDir = makeTempOutputDir(t);
...
test('UX simulation temp output uses a fresh generatedAt only when no prior report exists', (t) => {
  const outputDir = makeTempOutputDir(t);
...
test('UX simulation temp output can mutate feature evidence without dirtying committed reports', (t) => {
  const outputDir = makeTempOutputDir(t);
  const beforeCommittedJson = fs.readFileSync(reportPath, 'utf8');
  const beforeCommittedMarkdown = fs.readFileSync(markdownReportPath, 'utf8');
