...
function makeNativeIapProductFixture({ availablePurchases = [], products = [] } = {}) {
  ...
  const iap = {
    ...
    async requestPurchase({ request }) {
      ...
      state.requestedGoogleProductIds.push(...request.google.skus);
...
test('remove-ads paywall is surfaced near an ad placement and wired to purchase helpers', () => {
  ...
  assert.match(paywallSource, /getRemoveAdsUnavailableCopy/);
  ...
  assert.match(paywallSource, /webStoreUnavailable:/);
  assert.match(paywallSource, /nativeReceiptValidatorUnavailable:/);
  ...
  assert.match(
    paywallSource,
    /unavailableCopy\?\.accessibilityHint \?\? copy\.buyAccessibilityHint/,
  );
  ...
  assert.match(
    paywallSource,
    /unavailableCopy\?\.accessibilityHint \?\? copy\.restoreAccessibilityHint/,
  );
