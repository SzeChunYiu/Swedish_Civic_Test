...
test('scheduled Vercel deploy has a site-only main trigger and deploy-hook live smoke gate', () => {
  ...
  assert.equal(pkg.scripts['test:site-live:unit'], 'node --test scripts/check-live-site.test.js');
  assert.match(pkg.scripts.test, /npm run test:site-live:unit/);
  ...
  assert.match(workflow, /SITE_LIVE_ASSET_MANIFEST_PATH:\s*site\/asset-manifest\.json/);
  ...
  assert.equal(fs.existsSync(path.join(repoRoot, 'site/asset-manifest.json')), true);
