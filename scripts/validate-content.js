...
const QUESTION_GOOD_FRIDAY_REMEMBERS_ENGLISH_NATURALNESS_PATTERNS = [
  /\bGood Friday remembers Jesus'? death\b/i,
];
...
function findQuestionGoodFridayEnglishNaturalnessIssue(question) {
  if (!(question.tags || []).includes('easter')) return null;

  const text = [question.questionEn, question.explanationEn].join(' ');
  return QUESTION_GOOD_FRIDAY_REMEMBERS_ENGLISH_NATURALNESS_PATTERNS.find((pattern) =>
    pattern.test(text),
  );
}
  ...
  questions.forEach((question, index) => {
    ...
    if (questionSchemaIsValid) {
      ...
      if (goodFridayEnglishNaturalnessIssue) {
        fail(`${label} uses non-idiomatic Good Friday English wording`);
      } else {
