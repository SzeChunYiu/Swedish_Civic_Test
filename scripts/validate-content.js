...
const EXPECTED_LEGAL_SWEDISH_COPY_STRINGS = 62;
...
function findQuestionGeneratedTrueFalseNaturalnessIssue(question) {
  ...
  const swedishMatch = findGeneratedTrueFalseNaturalnessPatternMatch(question.questionSv);
  if (swedishMatch) return { ...swedishMatch, fieldName: 'questionSv' };

  const englishMatch = findGeneratedTrueFalseNaturalnessPatternMatch(question.questionEn);
  if (englishMatch) return { ...englishMatch, fieldName: 'questionEn' };
  ...
  questions.forEach((question, index) => {
    ...
    if (questionSchemaIsValid) {
      ...
      if (generatedTrueFalseNaturalnessIssue) {
        fail(
          `${label} contains a generated true/false naturalness issue in ${generatedTrueFalseNaturalnessIssue.fieldName}: ${formatGeneratedTrueFalseNaturalnessPatternMatch(generatedTrueFalseNaturalnessIssue)}`,
        );
      } else {
