...
const QUESTION_GENERATED_TRUE_FALSE_NATURALNESS_PATTERNS = [
  ...
  /\bdescribes that\b/i,
  /\bis\s+(?:be|judge)\b/i,
  /\bmain responsibility [^.?!]* is to be responsible\b/i,
...
function englishMainResponsibilityAnswer(value) {
...
function civicStatementEn(source, option) {
  ...
  if (match) {
    return `The main responsibility of ${lowerLeadingEnglishArticle(match[1])} is ${englishMainResponsibilityAnswer(answer)}`;
  }
