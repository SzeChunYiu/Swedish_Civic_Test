...
const { countQuestionBankProvenance } = require('./questionBankProvenanceCounts');
const {
...
const EXPECTED_HOME_ROUTE_COPY_LABELS = {
  sv: [
    ...
    '${count} kapitel',
    'Snabbstart',
    'Välj den väg som passar dagens energi.',
    'Svara på några frågor direkt och få källstödd återkoppling.',
    'Öppna kapitelöversikten och välj området du vill stärka.',
    'Gå tillbaka till bokmärkta och missade frågor innan de glöms bort.',
    'Sök bland frågor, svar och förklaringar',
    'Hitta ett begrepp, en fråga eller en förklaring utan att bläddra.',
    'Sök i frågorna',
    'Träna med tid när du vill känna hur ett helt provpass flyter.',
  ...
  en: [
    ...
    '${count} chapters',
    'Quick start',
    "Pick the route that fits today's energy.",
    'Answer a few questions now and get source-backed feedback.',
    'Open the chapter overview and choose the area you want to strengthen.',
    'Return to bookmarked and missed questions before they fade.',
    'Search questions, answers, and explanations',
    'Find a concept, question, or explanation without browsing.',
    'Search questions',
    'Use a timed run when you want the shape of a full test session.',
    'Focused review',
...
const EXPECTED_HOME_ROUTE_COPY_SNIPPETS = [
  ...
  ['title: copy.startPractice', 'home practice link must render localized copy'],
  [
    'accessibilityLabel: copy.browseChaptersAccessibilityLabel',
    'home chapter link must expose localized accessibility copy',
  ],
  ['title: copy.browseChapters', 'home chapter link must render localized copy'],
  ...
  ['title={copy.quickActionsTitle}', 'home quick-start section title must render localized copy'],
  ['subtitle={copy.quickActionsSubtitle}', 'home quick-start subtitle must render localized copy'],
  ['const quickActions = [', 'home quick-start actions must be generated from one route list'],
  ['body: copy.quickActionPracticeBody', 'home quick-start practice body must localize'],
  ['body: copy.quickActionLearnBody', 'home quick-start learn body must localize'],
  ['body: copy.quickActionReviewBody', 'home quick-start review body must localize'],
  ['body: copy.quickActionSearchBody', 'home quick-start search body must localize'],
  ['body: copy.quickActionExamBody', 'home quick-start exam body must localize'],
  ["href: '/search'", 'home quick-start actions must include the search route'],
  [
    'accessibilityLabel: copy.quickActionSearchAccessibilityLabel',
    'home quick-start search action must expose localized accessibility copy',
  ],
  ['title: copy.quickActionSearchTitle', 'home quick-start search action must localize'],
  ['<View style={styles.quickActionGrid}>', 'home quick-start grid must render'],
  [
    '<Text accessibilityRole="header" style={styles.quickActionTitle}>',
    'home quick-start card titles must be headers',
  ],
  ['minHeight: space[6]', 'home quick-start links must preserve 44px targets'],
...
const EXPECTED_HOME_ROUTE_HEADERS = [
  ...
  {
    label: 'quick-start section title',
    pattern: /<SectionHeader[\s\S]*\btitle=\{copy\.quickActionsTitle\}/,
  },
  {
...
let questionBankProvenanceCountsValidated = 0;
let questionBankProvenanceUhrQuestions = 0;
let questionBankProvenanceDerivedQuestions = 0;
let questionBankProvenanceEditorialQuestions = 0;
let questionBankProvenanceCompositionValidated = false;
...
function validateQuestionBankProvenanceComposition() {
  ...
  questionBankProvenanceCountsValidated = result.counts.total;
  questionBankProvenanceUhrQuestions = result.counts.uhr;
  questionBankProvenanceDerivedQuestions = result.counts.derived;
  questionBankProvenanceEditorialQuestions = result.counts.editorial;

  result.failures.forEach(fail);

  if (result.isValid) {
    questionBankProvenanceCompositionValidated = true;
  }
}
...
validateQuestionBankProvenanceComposition();
  ...
  JSON.stringify(
    {
      ...
      questionBankProvenanceCountsValidated,
      questionBankProvenanceUhrQuestions,
      questionBankProvenanceDerivedQuestions,
      questionBankProvenanceEditorialQuestions,
      questionBankProvenanceCompositionValidated,
