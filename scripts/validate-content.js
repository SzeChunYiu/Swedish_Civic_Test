...
const EXPECTED_PROFILE_ROUTE_COPY_SNIPPETS = [
  ...
  [
    ...
    'profile route must import the Pro runtime scope gate',
  ],
  [
    'const proRuntimeScopeEnabled = isProRuntimeScopeEnabled();',
    'profile route must read the Pro runtime scope gate',
  ],
  [
  ...
  ['<ProPaywall', 'profile route must keep the opt-in Pro tier comparison surface available'],
  [
...
const EXPECTED_HOME_ROUTE_COPY_LABELS = {
  sv: [
    ...
    'Starta ett övningsprov för att kontrollera din redoindikator',
    'Gör ett övningsprov',
...
const EXPECTED_HOME_ROUTE_COPY_SNIPPETS = [
  ...
  [
    ...
    'home route must derive the preparation signal from stored progress',
  ],
  ...
  [
    ...
    'home route preparation signal verdict must use localized copy',
  ],
  [
...
const EXPECTED_ABOUT_THE_TEST_ROUTE_COPY_SNIPPETS = [
  ...
  ['copy.sectionMaterialTitle', 'about-the-test material title must render localized copy'],
  ['copy.sectionIndependenceTitle', 'about-the-test official status must render localized copy'],
  [
  ...
  ['{copy.openPractice}', 'about-the-test practice link must render localized copy'],
  [
    'accessibilityLabel={copy.backHomeAccessibilityLabel}',
    'about-the-test home link must expose localized accessibility copy',
  ],
  ['{copy.backHome}', 'about-the-test home link must render localized copy'],
  ['<QuestionDisclaimer />', 'about-the-test route must render the official-test disclaimer'],
];
const SWEDISH_MOCK_EXAM_WORDING_FORBIDDEN_TERMS = [
  ['mock', 'prov'].join(''),
  ['mock', '-provet'].join(''),
];
const SWEDISH_MOCK_EXAM_WORDING_SCAN_ROOTS = ['app', 'components', 'lib', 'scripts', 'tests'];
const SWEDISH_MOCK_EXAM_WORDING_SCAN_EXTENSIONS = new Set([
  '.cjs',
  '.js',
  '.jsx',
  '.mjs',
...
let swedishMockExamWordingNaturalnessValidated = false;
...
function collectSwedishMockExamWordingScanFiles(relativeRoot) {
  ...
  if (rootStats.isFile()) return [absoluteRoot];
  ...
  entries.forEach((entry) => {
    ...
    if (entry.isDirectory()) {
      const childRoot = path.relative(repoRoot, absolutePath);
      collected.push(...collectSwedishMockExamWordingScanFiles(childRoot));
      return;
    }
    if (entry.isFile() && SWEDISH_MOCK_EXAM_WORDING_SCAN_EXTENSIONS.has(path.extname(entry.name))) {
...
function sourceContainsSwedishMockExamWording(source, term) {
  const escapedTerm = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`(^|[^\\p{L}\\p{N}_])${escapedTerm}(?![\\p{L}\\p{N}_])`, 'iu').test(source);
}

function validateSwedishMockExamWordingNaturalness() {
  let valid = true;
  ...
  SWEDISH_MOCK_EXAM_WORDING_SCAN_ROOTS.forEach((relativeRoot) => {
    collectSwedishMockExamWordingScanFiles(relativeRoot).forEach((filePath) => {
  ...
  scannedFiles.forEach((filePath) => {
    ...
    try {
      ...
      reject(
        `could not scan ${path.relative(repoRoot, filePath)} for legacy Swedish wording: ${error.message}`,
      );
      return;
    }

    SWEDISH_MOCK_EXAM_WORDING_FORBIDDEN_TERMS.forEach((term) => {
      if (sourceContainsSwedishMockExamWording(source, term)) {
        reject(
          `${path.relative(
            repoRoot,
            filePath,
          )} contains legacy Swedish mock-exam wording; use övningsprov wording instead`,
        );
      }
    });
  });

  if (valid && scannedFiles.size > 0) {
    swedishMockExamWordingNaturalnessValidated = true;
  }
}
...
validateSwedishMockExamWordingNaturalness();
  ...
  JSON.stringify(
    {
      ...
      swedishMockExamWordingNaturalnessValidated,
