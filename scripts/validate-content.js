...
const QUESTION_LUCIA_ROLE_EXPLANATION_NATURALNESS_PATTERNS = [
  /\bdär en person är Lucia,\s*bär ljuskrona\b/i,
  /\bwhere one person is Lucia,\s*wears a crown of lights\b/i,
];
...
function findQuestionLuciaRoleExplanationNaturalnessIssue(question) {
  const text = [question.explanationSv, question.explanationEn].join(' ');

  return QUESTION_LUCIA_ROLE_EXPLANATION_NATURALNESS_PATTERNS.find((pattern) => pattern.test(text));
}
...
let questionLuciaRoleExplanationNaturalnessValidated = 0;
...
function sourceAndGeneratedRowsForSourceId(sourceId) {
  if (!Array.isArray(sourceQuestions) || !Array.isArray(generatedPublishedQuestions)) return [];
...
function validateLuciaRoleExplanationNaturalness() {
  const rows = sourceAndGeneratedRowsForSourceId('q100');
  if (rows.length !== GENERATED_VARIANTS_PER_SOURCE + 1) {
    fail(
      `q100 Lucia explanation naturalness guard found ${rows.length} rows, expected ${
  ...
  rows.forEach((question) => {
    const issue = findQuestionLuciaRoleExplanationNaturalnessIssue(question);
    if (issue) {
      fail(`${question.id} uses Lucia role-scaffold explanation wording`);
    } else {
      questionLuciaRoleExplanationNaturalnessValidated += 1;
    }
  });
}
  ...
  questions.forEach((question, index) => {
    ...
    if (questionSchemaIsValid) {
      ...
      if (luciaExplanationRoleScaffoldIssue) {
        fail(`${label} uses scaffold Lucia explanation role wording`);
      } else {
...
validateLuciaRoleExplanationNaturalness();
  ...
  JSON.stringify(
    {
      ...
      questionLuciaRoleExplanationNaturalnessValidated,
