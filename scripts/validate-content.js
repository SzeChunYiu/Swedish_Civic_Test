...
function validateCitizenshipTimeline() {
  ...
  if (
    ...
    rejectDate(
      'civic knowledge test dates must stay after the citizenship rules date and in order',
    );
  }
...
function validateCountdownBannerHomeMount() {
  ...
  if (
    ...
    reject('Home route must import CountdownBanner');
  }

  if (/<CountdownBanner\s+language=\{language\}\s*\/>/.test(homeRouteSource)) {
    rulesValidated += 1;
  } else {
    reject('Home route must mount CountdownBanner with the selected language');
  }

  return { rulesValidated, parity };
}
...
if (process.argv.includes('--focus-countdown-banner')) {
  ...
  const homeMountValidation = validateCountdownBannerHomeMount();
  countdownBannerHomeMountRulesValidated = homeMountValidation.rulesValidated;
  countdownBannerHomeMountParityValidated = homeMountValidation.parity;
  exitWithValidationFailures();
  printValidationSummary({
