...
function validateLegalRouteHeaderParity() {
  ...
  for (const expectedRoute of EXPECTED_LEGAL_ROUTE_HEADERS) {
    ...
    if (
      !/import\s+\{[\s\S]*\bLegalPage\b[\s\S]*\bLegalSection\b[\s\S]*\}\s+from '..\/components\/compliance\/LegalPage';/.test(
        routeSource,
      )
    ) {
...
function validateUhrSourceMaterialLinkParity() {
  ...
  if (!/<LegalExternalLink[\s\S]*href=\{UHR_ABOUT_TEST_URL\}/.test(sourcesRoute)) {
    reject('app/sources.tsx must render the UHR about-test URL through LegalExternalLink');
  }
