...
function smtApplyAdPersonalization(choice) {
  const queue = (window.adsbygoogle = window.adsbygoogle || []);
  if (choice === "min") queue.requestNonPersonalizedAds = 1;
  else if (choice === "all") queue.requestNonPersonalizedAds = 0;
}

function smtApplyConsent(choice) {
  // 'all' = personalised, 'min' = non-personalised (NPA=1)
  if (choice === "all" || choice === "min") {
    smtApplyAdPersonalization(choice);
