...
(function () {
    ...
    stage.innerHTML = `
      <div class="hub">
        <div class="hub__stats">
          ...
          <div>
            <span class="hub__statlabel">${sv ? "Tidsatt övning" : "Timed practice"}</span>
        ...
        <ul>
          ${history.map((m) => `
            <li>
              ...
              <span class="mock-history__verdict">${sv ? "Lokalt resultat" : "Local result"}</span>
            </li>`).join("")}
        </ul>
      </div>` : "";
    ...
    stage.innerHTML = `
      <div class="mock-landing">
        <div class="mock-landing__inner">
          <span class="eyebrow">${sv ? "Tidsatt övning" : "Timed practice"}</span>
          <h1 class="practice__title">
            <span>${sv ? "Bygg ditt övningspass." : "Build your practice round."}</span>
          </h1>
          ...
          <div class="mock-cfg">
            ...
            <div class="mock-cfg__row">
              ...
              <div class="mock-cfg__hint">${sv ? "Timer endast för övning" : "Practice timer only"}</div>
            </div>
            ...
            <div class="mock-cfg__meta">
              <span><b>${sv ? "Resultat" : "Result"}</b> ${sv ? "procent rätt" : "percent correct"}</span>
          ...
          <div class="mock-landing__cta">
            <a class="btn btn--gold" href="#/mock?run=1" id="cfg-start">${sv ? "Starta tidsatt övning" : "Start timed practice"} →</a>
  ...
  function renderMockResult() {
    ...
    const correct = MOCK.questions.reduce((s, q, i) => s + (MOCK.answers[i] === q.answer ? 1 : 0), 0);
    const pct = Math.round((correct / total) * 100);
    const perfectPractice = correct === total;
    ...
    stage.innerHTML = `
      <div class="mock-result">
        ...
        <h2 class="mock-result__verdict">${sv ? "Övningspass klart." : "Practice round complete."}</h2>
    ...
    if (window.smtFx) {
      ...
      if (perfectPractice) {
        ...
        if (window.smtBuddyCelebrate) window.smtBuddyCelebrate(
          "Alla rätt. Stark övningsrunda.", "All correct. Strong practice round."
        );
      } else if (window.smtBuddyConsole) {
        window.smtBuddyConsole(
          "Review weak chapters before another timed practice.",
          "Repetera svaga kapitel före nästa tidsatta övning."
        );
      }
    }
  }
