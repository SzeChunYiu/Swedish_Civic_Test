    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 4 }}>
      {items.map(([key, p]) => {
        ...
        return (
          ...
          >
            <div
              style={{ display: 'flex', gap: 3, height: 22, borderRadius: 5, overflow: 'hidden' }}
            >
            ...
            <div style={{ fontSize: 10, opacity: 0.65, lineHeight: 1.3 }}>{desc}</div>
          </button>
        );
      })}
    </div>
...
function SMTTweaks() {
  ...
  useEffect(() => {
    applyHeadlineSize(t.headlineSize);
  }, [t.headlineSize]);
  useEffect(() => {
    applyFlagMotif(t.showFlagMotif);
  }, [t.showFlagMotif]);
  useEffect(() => {
    applySlogan(t.sloganVariant);
  }, [t.sloganVariant]);
  useEffect(() => {
    if (window.smtSetAdsMode) window.smtSetAdsMode(t.adsMode);
  }, [t.adsMode]);
...
ReactDOM.createRoot(document.body.appendChild(document.createElement('div'))).render(<SMTTweaks />);
