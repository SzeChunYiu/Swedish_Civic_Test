...
test('exam route shell and review copy follows the persisted settings language', () => {
  ...
  assert.match(source, /completionRetryLabel: 'Försök spara igen'/);
  assert.match(source, /completionRetryLabel: 'Retry saving result'/);
  assert.match(source, /accessibilityLabel=\{copy\.completionRetryAccessibilityLabel\}/);
  assert.match(source, /onPress=\{handleRetryCompletionSave\}/);
  assert.match(source, /setCompletionSaveFailed\(true\)/);
  assert.doesNotMatch(
    source,
