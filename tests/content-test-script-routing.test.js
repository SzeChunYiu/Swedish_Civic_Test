...
test('Flashcard accessibility focused content validation runs only its parity summary', () => {
  const result = spawnSync(
    process.execPath,
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout);
  const match = result.stdout.match(/\{[\s\S]*\}/);
  assert.ok(match, 'focused Flashcard accessibility validation should print JSON summary');
  const summary = JSON.parse(match[0]);
