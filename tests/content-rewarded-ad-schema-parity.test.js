...
test('rewarded ad schema parity rejects confirmation option drift', () => {
  const result = spawnSync(
    process.execPath,
    [
      '-e',
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/lib/monetization/rewardedAd.ts')) {
    return originalReadFileSync
      .call(this, filePath, ...args)
      .replace('confirmReward?: RewardedExtraExamRewardConfirmation;', 'confirmReward: RewardedExtraExamRewardConfirmation;');
  }
  return originalReadFileSync.call(this, filePath, ...args);
};
process.argv.push('--focus-rewarded-exam-schema');
...
`,
  ...
  assert.match(
    `${result.stdout}\n${result.stderr}`,
    /lib\/monetization\/rewardedAd\.ts RewardedExtraExamAdOptions\.confirmReward optional=false, expected true/,
  );
});
