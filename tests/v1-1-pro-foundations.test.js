...
test('perChapterProgress clamps malformed questionCount and strict boolean correctness', () => {
  const {
    dashboardSummary,
    mistakeConvergence,
    perChapterProgress,
  ...
  const questionChapterIndex = {
    ...
    'undersized-1': 'undersized',
    'undersized-2': 'undersized',
    'wrong-then-invalid': 'summary',
  };
  const bars = perChapterProgress(
    ...
    [
      ...
      { id: 'zero', questionCount: 0 },
    ],
    questionChapterIndex,
    { now },
  );
  const byId = new Map(bars.map((bar) => [bar.chapterId, bar]));

  assert.equal(byId.get('nan').coverage, 0);
  assert.equal(byId.get('nan').accuracy, 0);
  assert.equal(byId.get('negative').coverage, 0);
  assert.equal(byId.get('negative').accuracy, 0);
  assert.equal(byId.get('fractional').coverage, 0);
  assert.equal(byId.get('fractional').accuracy, 0);
  assert.equal(byId.get('undersized').coverage, 1);
  assert.equal(byId.get('undersized').accuracy, 1);
  assert.equal(byId.get('zero').coverage, 0);
  assert.ok(
    bars.every((bar) => Number.isFinite(bar.coverage) && bar.coverage >= 0 && bar.coverage <= 1),
  );

  const progress = progressWithSessions(sessions);
  const activeTimeOfDayBin = timeOfDayPattern(progress, { now }).find((bin) => bin.answers === 7);
  assert.equal(xpSparkline(progress, { daysBack: 1, now })[0].xp, 20);
  assert.equal(activeTimeOfDayBin.accuracy, 2 / 7);
  assert.equal(mistakeConvergence(progress, { daysBack: 2, now }).at(-1).unresolvedMistakes, 2);
  assert.equal(dashboardSummary(progress, questionChapterIndex, { now }).unresolvedMistakes, 2);
});
