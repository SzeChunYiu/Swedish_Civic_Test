...
test('streakWithFreeze: contiguous active days → streak counts them all, no freeze used', () => {
  const { calculateStreakWithFreeze, createInitialFreezeState } = loadTs('lib/learning/streakWithFreeze.ts');
...
function progressWithSessions(sessions) {
  return {
    totalXp: 0, level: 1, currentStreak: 0, dailyGoalAnswers: 10,
    questionProgress: {}, sessions,
  };
}
...
test('chapterWeaknesses: chapter with 0 answers gets isSparse + neutral effective accuracy', () => {
  ...
  const result = chapterWeaknesses({
    ...
    chapters: [{ id: 'a', questionCount: 10 }, { id: 'b', questionCount: 10 }],
...
test('topWeakChapters: ranks lower-accuracy chapter as weaker', () => {
  ...
  for (let i = 0; i < 10; i += 1) {
    answers.push({ questionId: `s${i}`, selectedOptionIds: [], isCorrect: true, answeredAt: '2026-05-18T10:00:00.000Z', timeSpentSeconds: 5 });
  }
  // chapter 'weak' — 10 answers, 3 correct
  for (let i = 0; i < 10; i += 1) {
    answers.push({ questionId: `w${i}`, selectedOptionIds: [], isCorrect: i < 3, answeredAt: '2026-05-18T10:00:00.000Z', timeSpentSeconds: 5 });
  }
  const idx = {};
  for (let i = 0; i < 10; i += 1) { idx[`s${i}`] = 'strong'; idx[`w${i}`] = 'weak'; }
  const top = topWeakChapters({
    progress: progressWithSessions([{ id: 's1', mode: 'study', questionIds: [], answers, startedAt: '2026-05-18T00:00:00.000Z' }]),
    chapters: [{ id: 'strong', questionCount: 20 }, { id: 'weak', questionCount: 20 }],
...
test('chapterWeaknesses: long-idle chapter gets staleness boost', () => {
  ...
  const answers = [];
  for (let i = 0; i < 10; i += 1) {
    answers.push({ questionId: `q${i}`, selectedOptionIds: [], isCorrect: true, answeredAt: '2026-01-01T10:00:00.000Z', timeSpentSeconds: 5 });
  }
  const idx = {};
  for (let i = 0; i < 10; i += 1) idx[`q${i}`] = 'stale';
  const result = chapterWeaknesses({
    progress: progressWithSessions([{ id: 's1', mode: 'study', questionIds: [], answers, startedAt: '2026-01-01T00:00:00.000Z' }]),
...
test('materializeMock: same mockId + same bank → deterministic question pick', () => {
  ...
  for (let i = 0; i < 100; i += 1) {
    bank.push({ id: `q${i}`, difficulty: i % 3 === 0 ? 'easy' : i % 3 === 1 ? 'medium' : 'hard', chapterId: `c${i % 5}` });
  }
  const a = materializeMock({ mockId: 'mock-2', bank });
  const b = materializeMock({ mockId: 'mock-2', bank });
  assert.deepEqual(a.questions.map((q) => q.questionId), b.questions.map((q) => q.questionId));
...
function fakeExamSession(answers) {
  return {
    id: 'exam-test', mode: 'exam', questionIds: answers.map((a) => a.questionId),
    answers, startedAt: '2026-05-19T09:00:00.000Z', completedAt: '2026-05-19T09:30:00.000Z',
  };
}
...
test('buildExamDiagnostic: overall accuracy + pass flag + per-chapter sort', () => {
  ...
  const answers = [
    ...
    ...Array.from({ length: 6 }, (_, i) => ({
      questionId: `c1q${i}`, selectedOptionIds: [], isCorrect: i < 4, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 10,
    })),
    // chapter c2: 2 of 8
    ...Array.from({ length: 8 }, (_, i) => ({
      questionId: `c2q${i}`, selectedOptionIds: [], isCorrect: i < 2, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 10,
    })),
  ];
  const idx = {};
  answers.forEach((a) => { idx[a.questionId] = a.questionId.startsWith('c1') ? 'c1' : 'c2'; });
  const diag = buildExamDiagnostic({ session: fakeExamSession(answers), questionChapterIndex: idx });
...
test('buildExamDiagnostic: high score → passed=true', () => {
  ...
  const answers = Array.from({ length: 25 }, (_, i) => ({
    questionId: `q${i}`, selectedOptionIds: [], isCorrect: i < 22, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 10,
  }));
  const idx = {}; answers.forEach((a) => { idx[a.questionId] = 'c1'; });
  const diag = buildExamDiagnostic({ session: fakeExamSession(answers), questionChapterIndex: idx });
...
test('buildExamDiagnostic: time-per-question + median populated', () => {
  ...
  const answers = [
    ...
    { questionId: 'q2', selectedOptionIds: [], isCorrect: false, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 15 },
    { questionId: 'q3', selectedOptionIds: [], isCorrect: true, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 30 },
  ];
...
test('formatPassLine: passing message in Sv + En', () => {
  ...
  const answers = Array.from({ length: 25 }, (_, i) => ({
    questionId: `q${i}`, selectedOptionIds: [], isCorrect: i < 22, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 5,
  }));
  const idx = {}; answers.forEach((a) => { idx[a.questionId] = 'c1'; });
  const diag = buildExamDiagnostic({ session: fakeExamSession(answers), questionChapterIndex: idx });
...
test('formatPassLine: failing message names the shortfall', () => {
  const { buildExamDiagnostic, formatPassLine } = loadTs('lib/learning/examDiagnostic.ts');
  const answers = Array.from({ length: 25 }, (_, i) => ({
    questionId: `q${i}`, selectedOptionIds: [], isCorrect: i < 10, answeredAt: '2026-05-19T09:00:00.000Z', timeSpentSeconds: 5,
  }));
  const idx = {}; answers.forEach((a) => { idx[a.questionId] = 'c1'; });
  const diag = buildExamDiagnostic({ session: fakeExamSession(answers), questionChapterIndex: idx });
  const en = formatPassLine(diag, 'en');
