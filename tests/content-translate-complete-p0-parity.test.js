...
test('TRANSLATE-COMPLETE P0 has explicit SV/EN completeness and naturalness closure evidence', () => {
  ...
  assert.equal(summary.glossaryNaturalnessTermsValidated, summary.glossaryTerms);
});

test('TRANSLATE-COMPLETE rejects stilted glossary naturalness wording', () => {
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/data/glossary.ts')) {
    return String(contents).replace('Rule of law', 'Legal certainty');
  }
  return contents;
};
...
`,
  ...
  assert.match(
    `${result.stdout}\n${result.stderr}`,
    /rattsakerhet termEn uses literal legal-certainty English wording/,
  );
});
