...
test('profile premium banner keeps current Remove Ads purchase and recovery contract', () => {
  ...
  assert.match(
    bannerSource,
    /const visibleStatus =[\s\S]*status === 'finish_failed'[\s\S]*adsDisabled[\s\S]*\? 'purchased'[\s\S]*purchaseUnavailable[\s\S]*\? 'unavailable'[\s\S]*: status;[\s\S]*const statusMessage =[\s\S]*nativePurchaseUnavailable[\s\S]*visibleStatus === 'unavailable'[\s\S]*\? copy\.nativeUnavailableStatus[\s\S]*: getStatusMessage\(visibleStatus, copy\)/,
  );
