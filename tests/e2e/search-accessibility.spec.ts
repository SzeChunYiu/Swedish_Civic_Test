...
async function seedSearchLanguage(page: Page, language: Language) {
  await page.addInitScript((seededLanguage) => {
    window.localStorage.setItem('language', seededLanguage);
...
for (const language of ['sv', 'en'] as const satisfies readonly Language[]) {
  test(`Search result summary announces changing counts in ${language.toUpperCase()}`, async ({
    ...
    await seedSearchLanguage(page, language);
