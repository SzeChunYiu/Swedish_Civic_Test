...
for (const scenario of streakFreezeCases) {
  test(`profile renders a rescued streak-freeze state in ${scenario.language}`, async ({
    page,
  }) => {
    ...
    const visibleProfileCopy = await page.locator('body').innerText();
    expect(visibleProfileCopy).not.toMatch(scenario.forbiddenVisibleCopy);
