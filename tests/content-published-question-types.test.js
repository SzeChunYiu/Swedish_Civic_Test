...
const goodFridayRemembersEnglishPattern = /\bGood Friday remembers Jesus'? death\b/i;
...
test('Good Friday source and exports use idiomatic commemorates English', () => {
  const generatedSiteBank = buildSiteQuestionBank().questions;
  const actualSiteBank = Array.from(actualStaticQuestions());
  const sourceQuestions = generatedSiteBank.filter(
    (question) => question.questionProvenance === 'uhr',
  ...
  const bankFindings = [...generatedSiteBank, ...actualSiteBank]
    ...
    .map((question) => question.id);
  const q101Rows = q101Ids.map((id) => generatedSiteBank.find((question) => question.id === id));
  const q101StaticRows = actualSiteBank.filter((question) => q101Ids.includes(question.id));

  assert.deepEqual(fileFindings, []);
  assert.deepEqual(bankFindings, []);
  assert.equal(q101Rows.length, 5);
  q101Rows.forEach((question, index) => {
    assert.ok(question, `q101 row ${q101Ids[index]} should be published`);
    assert.match(
      question.why.en,
      /Good Friday commemorates Jesus' death and Easter Sunday celebrates his resurrection/,
    );
  });
  assert.equal(q101StaticRows.length, 5);
  q101StaticRows.forEach((question) => {
    assert.match(
      question.why.en,
      /Good Friday commemorates Jesus' death and Easter Sunday celebrates his resurrection/,
    );
...
`,
  ...
  assert.notEqual(result.status, 0);
  assert.match(output, /q101 uses non-idiomatic Good Friday English wording/);
  assert.match(output, /q561 uses non-idiomatic Good Friday English wording/);
});
