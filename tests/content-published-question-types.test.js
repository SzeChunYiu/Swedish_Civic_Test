...
test('generated true/false question banks omit standalone reason fragments', () => {
  const generatedSiteBank = buildSiteQuestionBank().questions;
  const actualSiteBank = actualStaticQuestions();
  const standaloneReasonFragmentPattern =
    /^(?:En anledning är(?: att)? Det|One reason is(?: that)? It)\b/i;

  function offenders(questions) {
    return Array.from(questions)
      .filter((question) => question.type === 'true_false')
      .filter(
        (question) =>
          standaloneReasonFragmentPattern.test(question.q.sv) ||
          standaloneReasonFragmentPattern.test(question.q.en),
      )
      .map((question) => question.id);
  }

  assert.deepEqual(offenders(generatedSiteBank), []);
  assert.deepEqual(offenders(actualSiteBank), []);
});
