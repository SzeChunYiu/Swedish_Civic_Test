...
test('published question schema rejects targetless generated reason stems', () => {
  const result = spawnSync(
    process.execPath,
    [
      '-e',
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/data/questions.ts')) {
    return String(contents)
      .replace(
        'Sveriges nordligaste del ligger norr om polcirkeln.',
        'En anledning är att falsk information kan skrämma människor från demokratisk debatt.',
      )
      .replace(
        "Sweden's northernmost part lies north of the Arctic Circle.",
        'One reason is that false information can scare people away from democratic debate.',
      );
  }
  return contents;
};
