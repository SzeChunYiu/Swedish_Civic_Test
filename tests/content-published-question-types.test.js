...
test('published question types stay answerable by quiz runtime', () => {
  ...
  assert.equal(summary.questionFreeMediaImportantRolesOptionShapeValidated, 3);
...
test('free-media source prompts ask the civic concept directly in exports', () => {
  ...
  const q045SectionPractice = generatedSiteBank.find(
    (question) => question.id === generatedQuestionId(sourceQuestions, 'q045', 'singleChoice'),
  );
  const q045Judgement = generatedSiteBank.find(
    (question) => question.id === generatedQuestionId(sourceQuestions, 'q045', 'judgement'),
  );
  const expectedOptionsSv = [
    'Att informera, möjliggöra samhällsdebatt och granska personer med makt',
    'Att ersätta politiska val',
    'Att styras av staten',
    'Att bara publicera underhållning',
  ];
  const expectedOptionsEn = [
    'To inform, enable public debate, and scrutinize people with power',
    'To replace political elections',
    'To be controlled by the state',
    'To publish only entertainment',
  ];
  const optionText = (question, field) => question.opts.map((option) => option[field]);
  ...
  assert.deepEqual(optionText(q045, 'sv'), expectedOptionsSv);
  assert.deepEqual(optionText(q045, 'en'), expectedOptionsEn);
  assert.ok(q045SectionPractice, 'q045 section-practice generated variant should be published');
  assert.deepEqual(optionText(q045SectionPractice, 'sv'), expectedOptionsSv);
  assert.deepEqual(optionText(q045SectionPractice, 'en'), expectedOptionsEn);
  ...
  assert.equal(
    q045False.q.sv,
    'I en demokrati har fria medier viktiga uppgifter: att ersätta politiska val.',
  );
  assert.equal(
    q045False.q.en,
    'In a democracy, free media play an important role: replacing political elections.',
  );
  assert.ok(q045Judgement, 'q045 judgement generated variant should be published');
  assert.deepEqual(optionText(q045Judgement, 'sv'), expectedOptionsSv);
  assert.deepEqual(optionText(q045Judgement, 'en'), expectedOptionsEn);
  assert.deepEqual(generatedOffenders, []);
  assert.deepEqual(actualOffenders, []);
  assert.deepEqual(csvOffenders, []);
});
...
test('free-media important-role prompts reject mixed option grammar shapes', () => {
  const result = spawnSync(
    process.execPath,
    [
      '-e',
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/data/additionalQuestions.ts')) {
    return String(contents)
      .replace('Att ersätta politiska val', 'De ska ersätta politiska val')
      .replace('To replace political elections', 'They should replace political elections');
  }
  return contents;
};
require('./scripts/validate-content.js');
`,
  ...
  assert.match(
    `${result.stdout}\n${result.stderr}`,
    /q045 mixes free-media important-role option grammar shapes/,
  );
});
