...
const luciaRoleScaffoldExplanationPattern =
  /\b(?:där en person är Lucia,\s*bär ljuskrona|where one person is Lucia,\s*wears a crown of lights)\b/i;
...
test('published question types stay answerable by quiz runtime', () => {
  ...
  assert.equal(
    summary.questionLuciaRoleExplanationNaturalnessValidated,
    summary.publishedQuestions,
  );
...
test('Lucia celebration explanation uses direct Lucia wording in exports', () => {
  const generatedSiteBank = buildSiteQuestionBank().questions;
  const actualSiteBank = Array.from(actualStaticQuestions());
  const sourceQuestions = generatedSiteBank.filter(
    (question) => question.questionProvenance === 'uhr',
  );
  const q100GeneratedIds = [
  ...
  const q100Ids = ['q100', ...q100GeneratedIds];
  const explanationText = (question) => [question.why?.sv, question.why?.en].join(' ');
  const generatedOffenders = generatedSiteBank
    .filter((question) => q100Ids.includes(question.id))
    .filter((question) => luciaRoleScaffoldExplanationPattern.test(explanationText(question)))
    .map((question) => question.id);
  const actualOffenders = actualSiteBank
    .filter((question) => q100Ids.includes(question.id))
    .filter((question) => luciaRoleScaffoldExplanationPattern.test(explanationText(question)))
  ...
  const csvOffenders = fs
    .readFileSync(path.join(repoRoot, 'content/question-bank.csv'), 'utf8')
    .split(/\r?\n/)
    .filter((line) => q100Ids.includes(line.match(/^"([^"]+)"/)?.[1]))
    .filter((line) => luciaRoleScaffoldExplanationPattern.test(line))
    .map((line) => line.match(/^"([^"]+)"/)?.[1]);
  ...
  assert.deepEqual(actualOffenders, []);
  assert.deepEqual(csvOffenders, []);
  assert.ok(q100, 'q100 should be published in the site bank');
  assert.equal(
    q100.why.sv,
    'Luciafirandet handlar mycket om att sprida ljus när det är som mörkast på året. Firandet hör till den 13 december och kan innehålla luciatåg där Lucia bär ljuskrona och sjunger särskilda sånger tillsammans med andra deltagare.',
  );
  assert.equal(
    q100.why.en,
    'The Lucia celebration is largely about spreading light when the year is at its darkest. The celebration is linked to 13 December and can include Lucia processions where Lucia wears a crown of lights and sings special songs with other participants.',
  );
});

test('Lucia celebration explanation guard rejects role-scaffold copy', () => {
  const result = spawnSync(
    process.execPath,
    [
      '-e',
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/data/additionalQuestions.ts')) {
    return String(contents)
      .replace(
        'luciatåg där Lucia bär ljuskrona och sjunger särskilda sånger tillsammans med andra deltagare.',
        'luciatåg där en person är Lucia, bär ljuskrona och sjunger särskilda sånger tillsammans med andra deltagare.',
      )
      .replace(
        'Lucia processions where Lucia wears a crown of lights and sings special songs with other participants.',
        'Lucia processions where one person is Lucia, wears a crown of lights, and sings special songs with other participants.',
      );
  }
  return contents;
};
...
`,
  ...
  assert.match(
    `${result.stdout}\n${result.stderr}`,
    /q100 uses Lucia role-scaffold explanation wording/,
  );
});
