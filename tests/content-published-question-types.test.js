...
test('published question schema rejects redundant main-responsibility true/false stems', () => {
  const result = spawnSync(
    process.execPath,
    [
      '-e',
...
fs.readFileSync = function readFileSync(filePath, ...args) {
  ...
  if (normalizedPath.endsWith('/data/questions.ts')) {
    return String(contents).replace(
      "Sweden's northernmost part lies north of the Arctic Circle.",
      "The main responsibility of Sweden's regions is to be responsible for health care.",
    );
  }
  return contents;
};
